...
//    }
    ...
    public static boolean checkAddBuilding(BuildingDTO buildingDTO) {
        ...
        if (!StringUtils.check(buildingDTO.getRentArea()))
            return false;
        if (!StringUtils.check(buildingDTO.getRentPriceDescription()))
            return false;

        if (!NumberUtils.checkNumber(buildingDTO.getNumberOfBasement()))
            return false;
        if (!NumberUtils.checkNumber(buildingDTO.getFloorArea()))
            return false;
        if (!NumberUtils.checkNumber(buildingDTO.getRentPrice()))
            return false;
    ...
    public BuildingDTO findById(Long id) {
        ...
        String rentArea = rentAreaEntities.stream().map(it -> it.getValue().toString())
    ...
    public int countTotalItem(List<BuildingSearchResponse> list) {
        int res = 0;
        for (BuildingSearchResponse it : list)
            res += buildingRepository.countTotalItem(it);
        return res;
    }
